//! # 08_cycle_check - On-device DSP cost measurement
//!
//! Runs the **real `bbx_dsp` blocks** (polyblep sawtooth `OscillatorBlock`
//! into a `LowPassFilterBlock`) inside the audio ISR and measures the worst-case
//! cycles per block with the DWT cycle counter, so you can hear the library's
//! own oscillator on hardware *and* read its CPU cost off the LED:
//!
...
//! rate). You should also *hear* a 220 Hz filtered sawtooth — if it sounds
//! clean, the polyblep pipeline survived the trip to hardware.
//!
//! ## Building & Flashing
//!
//! ```bash
...
//! cargo run --example 08_cycle_check --release
//! ```
...
mod app {
    ...
    fn main() -> ! {
        // Hand-rolled init (mirrors board::init_audio for the Seed) because this
        ...
        // AK4556 (original Seed): release from power-down via PB11.
        ...
        // User LED on PC7 for the verdict readout.
