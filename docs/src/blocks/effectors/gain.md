...
let gain = builder.add_gain(0.0, Some(0.5));  // 0 dB + 50% = -6 dB effective
```
...
// Processing
let effect = builder.add_overdrive(3.0, 1.0, 0.5);
...
let lfo = builder.add_lfo(4.0, 0.5, None);  // 4 Hz tremolo
...
let gain = builder.add_gain(0.0, Some(0.5));
